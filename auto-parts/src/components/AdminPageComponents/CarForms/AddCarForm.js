...
const AddCarForm = () => {
  ...
  const submitAddCarForm = async data => {
    try {
      const res = await axios.post(
        process.env.REACT_APP_API_URL + '/CreateCar',
        {
          make: data.carMake,
          model: data.carModel,
  ...
  return (
    <Grid container style={mainGridStyle}>
      <Paper elevation={10} style={paperStyle}>
        <h2>Add a new car</h2>
        <form id="addCarForm" onSubmit={handleSubmit(submitAddCarForm)}>
        ...
        <Button
          ...
          color="primary"
          variant="contained"
          onClick={handleSubmit(submitAddCarForm)}
          fullWidth
          style={btnStyle}
        >
          Add car
